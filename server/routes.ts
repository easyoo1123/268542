...
import { insertTradeSchema, insertBankAccountSchema, trades, bankAccounts, type User, type InsertUser, type LoginData } from "../shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/admin/users", isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      const userData = {
        ...
        role: role || 'user', // ถ้าไม่ได้กำหนด ให้เป็น user ปกติ
        // ไม่ต้องกำหนด balance เพราะจะใช้ค่า default 0 บาทจากสคีมา
      };
      
      const newUser = await storage.createUser(userData);
      
      // ลบพาสเวิร์ดออกจากข้อมูลที่จะส่งกลับ
  ...
  app.patch("/api/bank-accounts/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      const bankAccount = await storage.getBankAccountById(bankAccountId);
      if (!bankAccount || bankAccount.userId !== req.user.id) {
        return res.status(403).json({ message: "You don't have permission to edit this bank account" });
      }
  ...
  // ดึงข้อมูลบัญชีธนาคารของผู้ใช้สำหรับแอดมิน
  app.get("/api/admin/users/:userId/bank-accounts", isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch user bank accounts" });
    }
  });
