...
export function registerSettingsRoutes(app: Express) {
  ...
  // สำรองข้อมูลเป็นไฟล์ JSON (ง่ายกว่า pg_dump และทำงานบนทุกแพลตฟอร์ม)
  app.post("/api/admin/settings/backup-json", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      // เรียกใช้สคริปต์ backup-database.ts
      const { exec } = await import('child_process');

      exec('npx tsx scripts/backup-database.ts', (error, stdout, stderr) => {
        if (error) {
        ...
        res.json({
          success: true,
          message: "สำรองข้อมูลเป็น JSON สำเร็จ",
          details: stdout,
          timestamp: new Date()
        });
      });
      ...
      res.status(500).json({
        success: false,
        message: "เกิดข้อผิดพลาดในการสำรองข้อมูลเป็น JSON",
      });
    }
  });
  
  // กู้คืนข้อมูลจากไฟล์ JSON
  app.post("/api/admin/settings/restore-json", isAuthenticated, isAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (!filename) {
        return res.status(400).json({
          success: false,
          message: "กรุณาระบุชื่อไฟล์สำรองข้อมูล",
        });
      }
      
      // เรียกใช้สคริปต์ restore-database.ts
      const { exec } = await import('child_process');

      exec(`npx tsx scripts/restore-database.ts ${filename}`, (error, stdout, stderr) => {
        if (error) {
        ...
        res.json({
          success: true,
          message: "กู้คืนข้อมูลจาก JSON สำเร็จ",
          details: stdout,
          timestamp: new Date()
        });
      });
      ...
      res.status(500).json({
        success: false,
        message: "เกิดข้อผิดพลาดในการกู้คืนข้อมูลจาก JSON",
      });
    }
  });
