...
import { Trade, Transaction } from "@shared/schema";
...
export default function ProfilePage() {
  ...
  // ดึงข้อมูลประวัติการฝาก-ถอนเงิน
    ...
    <MobileContainer>
      ...
      <div className="px-4 pb-20 mt-4">
        ...
        <Accordion type="single" collapsible className="w-full">
          ...
          {/* ส่วนโปรโมชั่น */}
          {/* ส่วนประวัติการฝาก-ถอนเงิน */}
          <AccordionItem value="transactions" className="mt-2">
            <AccordionTrigger className="px-3 py-3 bg-muted-foreground/5 rounded-lg font-medium">
              <div className="flex items-center">
                <History className="mr-2 h-5 w-5" />
                <span>ประวัติการฝาก-ถอนเงิน</span>
              </div>
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-4">
                  ...
                  {isTransactionsLoading ? (
                    ...
                    <div className="divide-y">
                      {transactions.map((transaction: Transaction) => (
                        <div className="p-3" key={transaction.id}>
                          <div className="flex justify-between items-center">
                            <div>
                              <div className="font-medium">
                                ...
                                <Badge 
                                  ...
                                  className={`ml-2 ${transaction.status === "approved" ? "bg-green-500 hover:bg-green-600" : ""}`}
                                >
                                  {transaction.status === "pending" && "รอดำเนินการ"}
                                  {transaction.status === "approved" && "สำเร็จ"}
                                  {transaction.status === "rejected" && "ถูกปฏิเสธ"}
                                </Badge>
                              </div>
                              <div className="text-sm text-muted-foreground">
                              ...
                              <div className="text-sm mt-1">
                                {transaction.method === "bank" && (
                                  <span>ผ่านธนาคาร {transaction.bankName}</span>
                                )}
                                {transaction.method === "promptpay" && (
                                  <span>PromptPay: {transaction.bankAccount}</span>
                                )}
                              </div>
                              {transaction.note && (
                            ...
                            <div className={`text-lg font-semibold ${transaction.type === "deposit" ? "text-green-600" : "text-red-600"}`}>
