...
// Component for editing bank account
export function EditBankAccountDialog({ 
  ...
  // Set initial values when account changes
  useEffect(() => {
    if (account) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            แก้ไขรายละเอียดบัญชีธนาคารของคุณ
          </DialogDescription>
        </DialogHeader>
        
        <div className="space-y-4 py-2">
          <div className="space-y-2">
            <Label htmlFor="edit-bank-name">ธนาคาร</Label>
            <select 
              id="edit-bank-name"
              className="w-full p-2 border rounded-md"
              value={bankName}
        ...
        <DialogFooter>
          ...
          >
            {editBankAccountMutation.isPending ? (
              <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> กำลังแก้ไขบัญชี</>
            ) : (
              "บันทึกการแก้ไข"
            )}
          </Button>
...
export function ManageBankAccountsDialog({
  ...
  const confirmDelete = () => {
    if (accountToDelete) {
      deleteBankAccountMutation.mutate(accountToDelete.id);
    }
  };
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              คุณต้องการลบบัญชีธนาคาร {accountToDelete?.bankName} {accountToDelete?.accountNumber} ใช่หรือไม่?
              <div className="mt-2 flex items-center text-destructive">
                <AlertTriangle className="h-4 w-4 mr-2" />
                <span>การลบบัญชีไม่สามารถเรียกคืนได้</span>
              </div>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>ยกเลิก</AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} disabled={deleteBankAccountMutation.isPending} className="bg-destructive text-destructive-foreground">
