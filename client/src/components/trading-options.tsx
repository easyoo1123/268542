...
export function TradingOptions({ crypto }: TradingOptionsProps) {
  ...
  // ตรวจสอบจำนวนเงินที่ป้อนไม่เกินยอดเงินคงเหลือในบัญชี
  const validateAndSetAmount = (value: string) => {
    ...
    // ตรวจสอบว่าไม่เกินยอดเงินคงเหลือในบัญชี
    if (user && numValue > parseFloat(user.balance)) {
      setAmount(user.balance); // จำกัดที่ยอดเงินคงเหลือในบัญชี
      toast({
        title: "ข้อจำกัดวงเงิน",
        description: "จำนวนเงินที่ระบุเกินยอดเงินคงเหลือในบัญชี",
        variant: "destructive",
      });
    } else {
      setAmount(value);
